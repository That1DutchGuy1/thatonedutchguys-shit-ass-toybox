document.addEventListener('DOMContentLoaded', () => {
  ...
  let achievedMilestones = JSON.parse(localStorage.getItem('achievedMilestones')) || []; // Track which milestones already fired this game
  ...
  function checkMilestone(val) {
    milestones.forEach(milestone => {
      if (val === milestone.value) {
        if (achievedMilestones.includes(milestone.value)) return; // Already popped this one, no repeats!
        achievedMilestones.push(milestone.value);
        saveAll();
  ...
  gambBtn.addEventListener('click', () => {
    ...
    counter -= 100;
    // Weighted random: 0 BUP is now the overwhelming favorite, everything else is rarer
    const roll = Math.random();
    let prize;
    if (roll < 0.577) {
      // 57.7%: ABSOLUTE ROBBERY, you get nothing
      prize = 0;
    } else if (roll < 0.827) {
      // 25%: 1-30 (still a rip-off)
      prize = Math.floor(Math.random() * 30) + 1;
    } else if (roll < 0.947) {
      // 12%: 31-80
      prize = Math.floor(Math.random() * 50) + 31;
    } else if (roll < 0.997) {
      // 5%: 81-100 (break even or slightly under)
      prize = Math.floor(Math.random() * 20) + 81;
    } else {
      // 0.3%: 101-500 ULTRA RARE JACKPOT
      prize = Math.floor(Math.random() * 400) + 101;
    }
    addScore(prize);
    updateScoreDisplay();
    saveAll();
    ...
    if (prize > 100) {
      ...
      showUpgradeMsg(`OH SNAP! You just got fucking scammed, bitch! You got 0 BUP!`);
    }
  });
